...
# txt 만들기
# index = 0
# for file_path in label_train_files: # label_train_files, label_valid_files
#     with open(label_train_path + file_path, 'r') as file: # label_train_path, label_valid_path
#         data = json.load(file)
#         json_str = data["FILE"][0]
#         file_name = json_str["FILE_NAME"]
#         items = json_str["ITEMS"][0]
...
#         else:
#             package = '0'
#         if items["CLASS"] != "적재불량" and items["CLASS"] != "정상차량":
...
#                 os.remove('img/' + file_path.replace('json', 'jpg'))
#             continue
#         box[2] = float(box[2]) / 1920
#         box[3] = float(box[3]) / 1080
#         box[0] = (float(box[0]) / 1920) + (box[2] / 2)
#         box[1] = (float(box[1]) / 1080) + (box[3] / 2)
#         text = package + ' ' + str(box[0]) + ' ' + str(box[1]) + ' ' + str(box[2]) + ' ' + str(box[3])
#         if (index == 0):
#             print(text)
#         file.close()
#         if os.path.isfile(image_train_path + file_path.replace('json', 'txt')): # image_train_path, image_valid_path
...
# for file_path in os.listdir(image_valid_path):
#     if file_path.endswith('jpg'):
#         img = Image.open(image_valid_path + file_path).convert("RGB")
#         new_img = img.resize((128, 128), Image.ANTIALIAS)
#         new_img.save(image_valid_path + file_path, format='jpeg', quality=100)
#         if index % 100 == 0:
#             print(index)
#     index += 1
...
# image_path = 'image_s/' # 'image', 'image_s'
# label_path = 'label_s/' # 'label', 'label_s'
...
#         if (index == 0):
#             print('0', file_name)
#         if os.path.exists('image_valid/' + file_name):
#             os.remove('image_valid/' + file_name)
#         if os.path.exists('label_valid/' + file_name.replace('jpg', 'json')):
#             os.remove('label_valid/' + file_name.replace('jpg', 'json'))
#         shutil.copy(image_path + file_name, 'image_valid/' + file_name)
#         shutil.copy(label_path + file_name.replace('jpg', 'json'), 'label_valid/' + file_name.replace('jpg', 'json'))
#     elif index % 10 >= 3: 
#         if (index == 4):
#             print('4', file_name)
#         if os.path.exists('image_train/' + file_name):
#             os.remove('image_train/' + file_name)
#         if os.path.exists('label_train/' + file_name.replace('jpg', 'json')):
#             os.remove('label_train/' + file_name.replace('jpg', 'json'))
#         shutil.copy(image_path + file_name, 'image_train/' + file_name)
#         shutil.copy(label_path + file_name.replace('jpg', 'json'), 'label_train/' + file_name.replace('jpg', 'json'))
#     index += 1
